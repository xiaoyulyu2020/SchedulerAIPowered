...
const RemoveShift: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        for (const date of selectedDates) {
            ...
            try {
                await EmployeeService.deleteShift(localISOTime, selectedEmployeeId);
            } catch (error) {
                toast.current?.show({ severity: 'error', summary: 'Error', detail: `Failed to remove shift for ${localISOTime}.` });
                console.error('Error removing shift:', error);
                return;
            }
        }

        toast.current?.show({ severity: 'success', summary: 'Success', detail: 'Shift(s) removed successfully.' });
        setTimeout(() => {
        ...
        <div style={{ width: '100%' }}>
            <form onSubmit={handleSubmit} className="p-fluid">
                ...
                <div style={{ paddingTop: '3rem', display: 'flex', justifyContent: 'center' }}>
                    <Button type="submit" label="Remove Shift" className="p-button-danger" />
                </div>
            </form>
            <Toast ref={toast} />
        </div>
